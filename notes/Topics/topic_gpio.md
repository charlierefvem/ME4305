...
> [!figure]
> ![Simplified GPIO output stage showing complementary high-side and low-side MOSFETs capable of sourcing or sinking current.](images/gpio/output_push_pull.svg)
> GPIO push-pull output stage with complementary MOSFETs.
...
> [!figure]
> ![Open-drain output stage with only the low-side MOSFET populated.](images/gpio/output_open_drain.svg)
> Open-drain output stage with only the low-side MOSFET populated.
...
> [!insight]
> Regardless of the output mode, a GPIO pin should be thought of as a switch rather than as a voltage source. The pin either connects the output to one of the supply rails or disconnects it; the external circuit determines how much current flows.
### Tri-State Outputs
...
> [!figure]
> ![Push-pull output followed by a tri-state buffer capable of placing the output in high impedance.](images/gpio/output_tri_state.svg)
> Push-pull output followed by a tri-state buffer capable of placing the output in high impedance.
...
> [!figure]
> ![GPIO input showing Schmitt trigger and protection diodes.](images/gpio/input.svg)
> GPIO input showing Schmitt trigger and protection diodes.
...
> Graph showing Schmitt trigger behavior.
...
> [!note]
> In the schematic snippet below, an external pull-up is shown, however most MCUs allow internal pull-ups to be enabled in firmware.

> [!figure]
> ![Example input circuits illustrating a pull-up resistor configuration.](images/gpio/pull_up.svg)
> Example input circuits illustrating a pull-up resistor configuration.
...
> [!note]
> Many motor drivers include internal pull-down resistors to implement this safety feature. Do not double-up with an external pull up in such a case.


> [!figure]
> ![Example input circuits illustrating a pull-down resistor configuration.](images/gpio/pull_down.svg)
> Example input circuits illustrating a pull-down resistor configuration.
...
> Example input circuit illustrating a wired-or configuration with a pull-up resistor.
...
> [!figure]
> ![Example GPIO circuit for driving an LED with a low side switch.](images/gpio/led_low_side.svg)
> Example GPIO circuit for driving an LED with a low side switch.

> [!note]
> For a low-side switch you should typically use an N-Channel MOSFET and for a high-side switch you should use a P-Channel MOSFET. These design choices will guarantee that the MCU is able to properly turn on and off the MOSFET.

> [!figure]
> ![Example GPIO circuit for driving an LED with a high side switch.](images/gpio/led_high_side.svg)
> Example GPIO circuit for driving an LED with a high side switch.
...
> GPIO driving an inductive load with appropriate flyback protection.
...
> [!figure]
> ![Example button circuit in active low configuration.](images/gpio/active_low_switch.svg)
> Active-low MCU switch with a pull-up resistor to VDD.

> [!note]
> It is generally preferred to configure MCU pins and switches in an active-low configuration because many microcontrollers are slightly better at sinking current than sourcing it, and active-low signals integrate naturally with pull-up resistors and open-drain logic.

> [!figure]
> ![Example button circuit in active high configuration.](images/gpio/active_high_switch.svg)
> Example button circuit in active high configuration.
