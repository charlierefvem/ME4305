...
> Shared open-drain I2C bus with one controller and two peripherals.
...
> Open-drain I2C physical layer with pull-up resistors.
...
> [!table]
> Example register addresses and contents.
>
...
> [!table]
> Read and write methods provided by `pyb.I2C`.
>
...
> [!note]
> In the example the data is converted into three signed 16-bit integer values, but in practice these values will also need to be scaled by a gain to convert the values into engineering units. The end result for each axis will likely be a floating point value.
...
> [!note]
> It is also allowed to use format strings like `'<3h'` to refer to three signed 16-bit integers in little-endian format. This style can be convenient in scenarios that require production of format strings programmatically.
