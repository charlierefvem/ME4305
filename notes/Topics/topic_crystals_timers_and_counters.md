...
> RC and Pierce oscillator circuits with a typical oscillator output.
...
> Crystal and PLL waveforms illustrating frequency multiplication.
...
> Timing diagram of a free-running up counter with autoreload and associated clock waveform.
...
> Comparison of timer operation with and without a prescaler showing reduced counting rate and longer period.
...
> Up-counting timer compared against two compare values producing left-aligned PWM waveforms with different duty cycles.

> [!note]
> In the preceding figure the stair-step waveform has been simplified to a standard sawtooth waveform. It should be imagined that zooming in on the sawtooth would reveal many small stair-steps as shown in previous figures in this lecture.
...
> Up-down counting timer producing center-aligned PWM waveforms that reduce phase asymmetry between channels.
...
> PWM and motor spectra illustrating the motor's low-pass response.
