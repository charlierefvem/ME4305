...
> [!table]
> Components of Python slice syntax.
>
...
> [!block_listing] Passing a `bytearray` slice
> ```python
...
> [!block_listing] Passing a `memoryview` slice
> ```python
> buf = bytearray([0, 1, 2, 3, 4])
>
> def mutate_last(b):
...
> [!block_listing] Using memory views in a BNO055 driver
> ```python
